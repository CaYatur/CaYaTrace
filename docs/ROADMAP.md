...
The WebView2 UI over the existing engine, in the CaYaDev visual language.

- Causal tree with virtualization — an installer run produces six figures of nodes
- Timeline scrubber, category filters, full-text search over targets
...
- Self-contained HTML report for readers who will not use the tool
- CSV per category, JSON for the whole session
- Redaction pass for sharing evidence outside the machine it came from
...
- Application-layer encryption (X25519 + ChaCha20-Poly1305), because a lab network usually
  has no usable PKI and self-signed TLS means trust-store changes on machines meant to be
  disposable
- Several transports so the user can pick what their setup allows
